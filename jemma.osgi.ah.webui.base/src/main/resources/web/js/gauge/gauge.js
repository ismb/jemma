/**
 * gauge.js 1.33 (06-Jan-2010) (c) by Christian Effenberger 
...
var gauge = { version : 1.33, released : '2010-01-06 16:42:00', 
	...
	add : function(object, options) {
		...
		defopts={"width":gauge.defaultWidth,
		...
		tmpId = (object.attr("id") != 'undefined' ?object.attr("id"):uniqueID());
		tmp=(id==''||id==null?object.attr("id")+'_gauge':id); 
		if(!document.getElementById(tmp)) {
			try {
				if(document.all&&document.namespaces&&!window.opera) {
					...
					var self=document.createElement(['<var style="zoom:1;display:'+dpl+
							';width:'+width+'px;height:'+height+'px;padding:0px;margin:0px;">'].join(''));
				...
				if(self||self.getContext("2d")) {
					...
					idTmp = id!=null?id:object.attr('id')+'_gauge';
					self.setAttribute('id', idTmp);
			...
			try {
				...
				if(document.all&&document.namespaces&&!window.opera&&self.tagName.toUpperCase()=="VAR") {
					...
					if(!self.ns) {
						...
						if(v) {
							for(i=0; i<k; i++) {
								a+=' m 100,'+parseInt((h*100)-(b*100)-(i*(b*100))+100)+' l '+parseInt((w*100)-100)+','+parseInt((h*100)-(b*100)-(i*(b*100))+100)+' e'; z+=' m 100,'+parseInt((h*100)-(b*100)-(i*(b*100)))+' l '+parseInt((w*100)-100)+','+parseInt((h*100)-(b*100)-(i*(b*100)))+' e';}
							}
						else {
							for(i=0; i<k; i++) {
								a+=' m '+parseInt((b*100)+(i*(b*100))+100)+',100 l '+parseInt((b*100)+(i*(b*100))+100)+','+parseInt((h*100)-100)+' e'; z+=' m '+parseInt((b*100)+(i*(b*100)))+',100 l '+parseInt((b*100)+(i*(b*100)))+','+parseInt((h*100)-100)+' e';
							}
						}
				...
				if(self.tagName.toUpperCase()=="CANVAS"&&self.getContext("2d")) {
