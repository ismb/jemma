...
CostiConsumi.GestFotoVoltaico = function() {
	...
	if ((CostiConsumi.mode == CostiConsumi.CONSUMI) || (CostiConsumi.mode == CostiConsumi.COSTI)) {
		...
		$("#CostoConsumoAttualeTitolo").css('top', '45%').css('left', '30%');
		...
		$("#IndicatoreTitolo").css('top', '78%').css('left', '0%').width('40%').height('10%');
		$("#Indicatore").css('top', '68%').css('left', '0%');
...
CostiConsumi.GetDatiProduzione = function() {
	...
	if (InterfaceEnergyHome.mode > 0) {
		...
		var powerProduction = Utils.leggiCookie('MaxPowerProduction');
...
CostiConsumi.VisGrafico = function() {
	...
	chartConsumi = new Highcharts.Chart({
		...
		tooltip : {
			formatter : function() {
				var txt = '<b>'+this.series.name +'</b>: ' + Highcharts.numberFormat(this.y, 1) + ' KWh<br/>' + 'alle ore: ' + this.x + ':00'
				return txt;
			}
		},
		series : [{
...
CostiConsumi.GetDatiProduzioneIAC = function() {
	...
	if (InterfaceEnergyHome.mode > 1) {
		...
		try {
			...
			//prendo Produzione e sommo
			//prendo vendita e sommo
			//faccio il rapporto e visualizzo la percentuale
...
CostiConsumi.DatiPercIAC = function(result, err){
	var perc = 0;
	//ToDo: Bisogna creare un metodo che chiama via Ajax il server per farsi restituire il valore IAC,
	//      essendo espresso in percentuale basta visualizzarlo direttamente sulla GUI.
	//SOLO PER TEST
	//perc = 68;
	if (err != null){
		if (Main.env == 0) console.log('exception in FotoVoltaico.js - in CostiConsumi.GetDatiVenditaIAC method: ', err);
