...
public class DataFreescale implements IDataLayer {
			...
			public void run() {
				...
				while (!destroy) {
					try {
						synchronized (listOfCommandToSend) {
							...
							if (_currentCommand != null) {
								if (gal.getPropertiesManager().getDebugEnabled()) {
									LOG.info(">>> Sending: " + _currentCommand.ToHexString());
								}
						...
						if (gal.getPropertiesManager().getDebugEnabled())
							LOG.error("Error Sending command: " + e.toString(),e);
					}
				}
			}
		};
				...
				while (!destroy) {
					if (!receivedDataQueue.isEmpty()) {
						try {
							...
							if (gal.getPropertiesManager().getDebugEnabled())
								LOG.error("Error on createMessageFromRowData: " + e.toString());
						...
						if (tempArray != null) {
							...
							try {
								...
								if (gal.getPropertiesManager().getDebugEnabled())
									LOG.error("Error on processMessages: " + e.toString());
	...
	private short[] createMessageFromRowData() {
		...
		while ((!receivedDataQueue.isEmpty()) && (receivedDataQueue.get(0) != DataManipulation.SEQUENCE_START)) {
			...
			if (gal.getPropertiesManager().getDebugEnabled())
				LOG.debug("Error on Message Received, removing wrong byte: " + String.format("%02X", _toremove));
		...
		if (currCscControl != messageCfc) {
			if (gal.getPropertiesManager().getDebugEnabled())
				LOG.debug("Error CscControl: " + String.format("%02X", currCscControl) + " != " + String.format("%02X", messageCfc));
		...
		if (serialDataError) {
			...
			for (int z = 0; z < toremove; z++) {
				if (receivedDataQueue.size() > 0) {
					...
					if (gal.getPropertiesManager().getDebugEnabled())
						LOG.debug("Removed Byte: " + String.format("%02X", extracted));
				}
			}
		...
		while (_size > 0) {
			...
			if (_command == FreescaleConstants.APSDEDataIndication) {
				...
				case 0x00:
					...
					if (gal.getPropertiesManager().getDebugEnabled()) {
						LOG.warn("Message Discarded: found reserved 0x00 as Destination Address Mode ");
				...
				default:
					if (gal.getPropertiesManager().getDebugEnabled()) {
						LOG.warn("Message Discarded: not valid Destination Address Mode");
					}
					// Error found, we don't proceed and discard the
					// message
					return;
						...
						if (gal.getPropertiesManager().getKeepAliveThreshold() > 0) {
							if (!gal.getNetworkcache().get(_indexOnCache).isSleepy()) {
								...
								if (gal.getPropertiesManager().getDebugEnabled()) {
									...
									// + "\n\r");
									LOG.trace("Postponing  timer Freshness by Aps.Indication for node:" + gal.getNetworkcache().get(_indexOnCache).get_node().getAddress().getNetworkAddress());
								}
							}
								...
								Runnable thr = new MyThread(address) {
									...
									public void run() {
										...
										if (_indexOnCache == -1) {
											...
											if (gal.getPropertiesManager().getDebugEnabled()) {
												LOG.info("AutoDiscoveryUnknownNodes procedure of Node:" + messageEvent.getSourceAddress().getNetworkAddress());
											}
											try {
												...
												if (gal.getPropertiesManager().getDebugEnabled())
													LOG.info("Sending IeeeReq to:" + _address.getNetworkAddress());
												ieee = readExtAddress(INTERNAL_TIMEOUT, _address.getNetworkAddress().shortValue());
												_address.setIeeeAddress(ieee);
												if (gal.getPropertiesManager().getDebugEnabled()) {
													LOG.info("Readed Ieee of the new node:" + _address.getNetworkAddress() + " Ieee: " + ieee.toString());
												}
												if (gal.getPropertiesManager().getDebugEnabled())
													LOG.info("Sending NodeDescriptorReq to:" + _address.getNetworkAddress());
												...
												if (gal.getPropertiesManager().getDebugEnabled()) {
													LOG.info("Readed NodeDescriptor of the new node:" + _address.getNetworkAddress());
												...
												LOG.error("Error on getAutoDiscoveryUnknownNodes for node:" + _address.getNetworkAddress() + " Error:" + e.getMessage());
				...
				default:
					if (gal.getPropertiesManager().getDebugEnabled()) {
						LOG.warn("Message Discarded: not valid Security Status");
					}
					// Error found, we don't proceed and discard the
					// message
					return;
			...
			/* INTERPAN-DATA.Indication */
				...
				case 0x00:
					// Reserved (No source address supplied)
					if (gal.getPropertiesManager().getDebugEnabled()) {
						LOG.warn("Message Discarded: found reserved 0x00 as Source Address Mode ");
					}// Error found, we don't proceed and discard the
						// message
					return;
				case 0x01:
				...
				default:
					if (gal.getPropertiesManager().getDebugEnabled()) {
						LOG.warn("Message Discarded: not valid Source Address Mode");
					}
					// Error found, we don't proceed and discard the
					// message
					return;
			...
			/* APSDE-DATA.Confirm */
				...
				synchronized (listLocker) {
					for (ParserLocker pl : listLocker) {
						...
						/*
						 * if (gal.getPropertiesManager().getDebugEnabled())
						 * LOG.info("APSDE-DATA.Confirm KEY SENT: " +
			...
			/* NLME-JOIN.Confirm */
				...
				//TODO Note by Riccardo to be Cheched with Marco: why should we leave the handling of debug behaviour to gal.getPropertiesManager() ?? Wouldn't it be better to leave this for the log configuration files ?
	...
	public Status APSME_SETSync(long timeout, short _AttID, String _value) throws GatewayException, Exception {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in APSME SET");
			}
	...
	public String APSME_GETSync(long timeout, short _AttID) throws Exception {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in APSME-GET.Request");
			}
	...
	public String NMLE_GetSync(long timeout, short _AttID) throws Exception {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in NLME-GET.Request");
			}
	...
	public Status stopNetworkSync(long timeout) throws Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZDP-StopNwkEx.Request");
			}
	...
	public Status sendApsSync(long timeout, APSMessage message) throws Exception {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.debug("Timeout expired in send aps message");
			}
	...
	public short configureEndPointSync(long timeout, SimpleDescriptor desc) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in Configure End Point");
			}
	...
	public ByteArrayObject makeByteArrayFromApsMessage(APSMessage apsMessage) throws Exception {
		...
		if (gal.getPropertiesManager().getDebugEnabled()) {
			LOG.debug("Write APS on: " + System.currentTimeMillis() + " Message:" + _res.ToHexString());
		}
		return _res;
	}
	...
	public ByteArrayObject makeByteArrayFromInterPANMessage(InterPANMessage message) throws Exception {
		...
		if (gal.getPropertiesManager().getDebugEnabled()) {
			LOG.debug("Write InterPanMessahe on: " + System.currentTimeMillis() + " Message:" + _res.ToHexString());
		}
		return _res;
	}
	...
	public Status startGatewayDeviceSync(long timeout, StartupAttributeInfo sai) throws IOException, Exception, GatewayException {
		...
		if (_statWriteSas.getCode() == 0) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.debug("StartupSet value read from PropertiesManager:" + gal.getPropertiesManager().getStartupSet());
				LOG.debug("StartupControlMode value read from PropertiesManager:" + gal.getPropertiesManager().getSturtupAttributeInfo().getStartupControl().byteValue());
			}
			...
			_res = Set_SequenceStart_And_FSC(_res, FreescaleConstants.ZTCStartNwkExRequest);/*
																							 ...
																							 */
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.debug("Start Network command:" + _res.ToHexString());
			}
			ParserLocker lock = new ParserLocker();
			...
			if (status.getCode() == ParserLocker.INVALID_ID) {
				if (gal.getPropertiesManager().getDebugEnabled()) {
					LOG.warn("Timeout expired in startGatewayDevice");
				}
	...
	private Status WriteSasSync(long timeout, StartupAttributeInfo sai) throws InterruptedException, Exception {
		...
		if (gal.getPropertiesManager().getDebugEnabled()) {
			LOG.debug("APS Use Extended PAN Id:" + DataManipulation.convertBytesToString(APSUseExtendedPANId));
		}
		...
		if (gal.getPropertiesManager().getDebugEnabled())
			LOG.debug("Channel read from PropertiesManager:" + sai.getChannelMask());
		...
		if (gal.getPropertiesManager().getDebugEnabled()) {
			LOG.debug("WriteSas Command:" + res.ToHexString());
		}
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.debug("Timeout expired in write sas");
			}
	...
	public short getChannelSync(long timeout) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZTC-GetChannel.Request");
			}
	...
	public BigInteger readExtAddressGal(long timeout) throws GatewayException, Exception {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZTC-ReadExtAddr.Request");
			}
	...
	public BigInteger readExtAddress(long timeout, short shortAddress) throws GatewayException, Exception {
		...
		// Control
		if (gal.getPropertiesManager().getDebugEnabled()) {
			LOG.debug("ZDP-IEEE_addr.Request.Request:" + _res.ToHexString());
		}
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZDP-IEEE_addr.Request");
			}
	...
	public NodeDescriptor getNodeDescriptorSync(long timeout, Address addrOfInterest) throws IOException, Exception, GatewayException {
		...
		_res = Set_SequenceStart_And_FSC(_res, FreescaleConstants.ZDPNodeDescriptorRequest);/*
																							 ...
																							 */
		if (gal.getPropertiesManager().getDebugEnabled()) {
			LOG.debug("ZDP-NodeDescriptor.Request:" + _res.ToHexString());
		}
	...
	public List<Short> startServiceDiscoverySync(long timeout, Address aoi) throws Exception {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.debug("Timeout expired in ZDP-Active_EP_req.Request");
			}
	...
	public Status clearEndpointSync(short endpoint) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in Deregister End Point");
			}
	...
	public NodeServices getLocalServices() throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in GetEndPointIdList");
			}
	...
	public ServiceDescriptor getServiceDescriptor(long timeout, Address addrOfInterest, short endpoint) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZDP-SimpleDescriptor.Request");
			}
	...
	public BindingList getNodeBindings(long timeout, Address addrOfInterest, short index) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZDP-Mgmt_Bind.Request");
			}
	...
	public Status addBinding(long timeout, Binding binding) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZDP-BIND.Response");
			}
	...
	public Status removeBinding(long timeout, Binding binding) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZDP-UNBIND.Response");
			}
	...
	public Status ClearDeviceKeyPairSet(long timeout, Address addrOfInterest) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ClearDeviceKeyPairSet");
			}
	...
	public Status ClearNeighborTableEntry(long timeout, Address addrOfInterest) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZTC-ClearNeighborTableEntry.Request");
			}
	...
	public Status NMLE_SETSync(long timeout, short _AttID, String _value) throws Exception {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in NMLE SET");
			}
	...
	public Mgmt_LQI_rsp Mgmt_Lqi_Request(long timeout, Address addrOfInterest, short startIndex) throws IOException, Exception, GatewayException {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in ZDP-Mgmt_Lqi.Request");
			}
	...
	public Status sendInterPANMessaSync(long timeout, InterPANMessage message) throws Exception {
		...
		if (status.getCode() == ParserLocker.INVALID_ID) {
			...
			if (gal.getPropertiesManager().getDebugEnabled()) {
				LOG.warn("Timeout expired in send InterPANMessage");
			}
