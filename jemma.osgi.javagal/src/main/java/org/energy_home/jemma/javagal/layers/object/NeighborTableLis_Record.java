...
public class NeighborTableLis_Record {
	...
	public NeighborTableLis_Record(byte[] _data) {
		...
		if (_data.length >= 21) {
			_Extended_PAN_Id = DataManipulation.toLong(_data[7], _data[6],
					_data[5], _data[4], _data[3], _data[2], _data[1], _data[0]);
			_Extended_Address = DataManipulation.toLong(_data[15], _data[14],
					_data[13], _data[12], _data[11], _data[10], _data[9],
					_data[8]);
