...
var getProfiles = function(){
	...
	$.each(myProfile, function(iCounter, vProfile){
		tmpString += "'" + vProfile.zigbeeprofileid + "':'" + vProfile.description + "',";
	});
...
var getClusters = function(devId){
	var tmpObj = {inputCM:'', outputCM:'', inputCO:'', outputCO:''};
...
function readProfileAndClusterFromXML(xmlpath, callback){
	var returnObj = {profile: {profilesid: null, 
	...
	$.ajax({
		...
		$(xml).find('profileinformation').each(function(){
			returnObj.profile.profilesid = $(this).find('profilesid').text();
			returnObj.profile.zigbeeprofileid = $(this).find('zigbeeprofileid').text();
			returnObj.profile.description = $(this).find('description').text();
	    });
		...
		$(xml).find('clusters').each(function(){
			var tmpClstr = {clustersidentifier: $(this).find('clustersidentifier').text(),
				 		   	description: $(this).find('description').text()};
