...
public class GalGuiHttpApplication extends DefaultWebApplication implements HttpImplementor, HttpContext {
	...
	private String applicationWebAlias = "/webguigal";
	...
	public void activate(BundleContext _bc, GatewayInterface _gatewayInterface, HttpService _httpService) {

		this.gatewayInterface = _gatewayInterface;
		this.bc = _bc;

		this.setHttpService(_httpService);
	...
	protected synchronized void setHttpService(HttpService s) {
		...
		this.registerResource("/servlet/net/default/channel", new channelServlet(this));
	...
	/*
	 * protected synchronized void setUserAdmin(UserAdmin s) { this.userAdmin =
	 * s; }
	 * 
	 * protected synchronized void unsetUserAdmin(UserAdmin s) { if
	 * (this.userAdmin == s) this.userAdmin = null; }
	 */
	...
	public boolean handleSecurity(HttpServletRequest request, HttpServletResponse response) throws IOException {
		 ...
		 * return true; } else if (queryString.equals(applicationWebAlias) ||
		 * (queryString.equals(applicationWebAlias + "/"))) {
		 * response.sendRedirect(applicationWebAlias + "/index.html"); return
		 * true; }
		 */
		if (enableSecurity) {
			if (useBasic) {
				...
				// String(Base64.decode(base64credentials.getBytes()));
				/*
				 * int colon = credentials.indexOf(':'); String userid =
				 * credentials.substring(0, colon); String password =
				 * credentials.substring(colon + 1);
				 * 
				 * Authorization subject = null;
				 * 
				 * try { subject = login(request, userid, password); } catch
				 * (LoginException e) { return failAuthorization(request,
				 * response); }
				 * 
				 * request.setAttribute(HttpContext.REMOTE_USER, userid);
				 * request.setAttribute(HttpContext.AUTHENTICATION_TYPE,
				 * authscheme); request.setAttribute(HttpContext.AUTHORIZATION,
				 * subject);
				 */
			} else {
				...
				if (queryString.startsWith(applicationWebAlias + "/conf")) {
					...
					if (submit != null) {
						...
						/*
						 * if (!allowUser(username, password)) { return
						 * redirectToLoginPage(request, response); } else {
						 */
						...
						if (queryString.equals(applicationWebAlias + "/conf/login.html")) {
							...
							if (done == null) {
								if (request.getMethod().equals("GET")) {
									return true;// redirectToLoginPage(request,
												// response);
								} else {
									response.sendError(HttpServletResponse.SC_FORBIDDEN);
									return false;
	...
	/*
	 * private boolean allowUser(String username, String password) { if
	 * (userAdmin != null) { User user =
	 * userAdmin.getUser("org.energy_home.jemma.username", username); if (user
	 * == null) { return false; } if
	 * (!user.hasCredential("org.energy_home.jemma.password", password)) {
	 * return false; }
	 * 
	 ...
	 * username, final String password) throws LoginException { Authorization a
	 * = (Authorization) request.getAttribute(HttpContext.AUTHORIZATION); if (a
	 * != null) { return a; }
	 * 
	 * if (userAdmin != null) { User user =
	 * userAdmin.getUser("it.telecomitalia.username", username); if (user ==
	 * null) { throw new LoginException(); } if
	 * (!user.hasCredential("it.telecomitalia.password", password)) { throw new
	 * LoginException(); }
	 * 
	 ...
	 * 
	 * public void modified(ComponentContext ctxt, Map props) { update(props); }
	 */
