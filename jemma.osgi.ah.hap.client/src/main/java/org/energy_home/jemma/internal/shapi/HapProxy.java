...
public class HapProxy extends HttpServlet implements IApplicationService, ISubscriptionManager {
	...
	public void setM2MHapService(IM2MHapService hapService) {
		...
		try {	
			...
			serviceClusterProxies = new IServiceCluster[] {onOffClusterProxy, meteringClusterProxy, applianceControlClusterProxy};
		} catch (Exception e) {
		...
		if (requestUri.startsWith(M2MConstants.URL_SCL_BASE)) {
			...
			try {
				...
				if (requestUri.startsWith(subscriptionItemsAddressedId)) {
					...
					scl.setOnLineStatus(hapService.getM2MHapService().isConnected() ? SclStatusEnumeration.ONLINE : SclStatusEnumeration.DISCONNECTED);
					if (sclItems != null)
